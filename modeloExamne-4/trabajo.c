...
int altaTrabajo( eTrabajo vec[],int tam,eServicio ser[], int tamSer,eBici mas[],int tamMas,int* idIngreso,eColor col[], int tamCol,eTipo tip[], int tamTip)
{
    ...
    if (vec != NULL && tam > 0 && ser!=NULL &&tamSer>0 && idIngreso!=NULL &&mas!=NULL && tamMas>0 )
    {
        ...
        if(espacioLibreBici(mas,tamMas)>=0)
        {
            ...
            do
            {
                utn_clear();
                printf("******** alta trabajo ********\n\n");
                mostrarBici(mas,tamMas,tip,tamTip,col,tamCol);
                utn_getInt(&idMarcotaIngreso,"Ingrese el id de una mascota ","error ingrese un numero valido");
                posicion=buscarBiciId(mas,tamMas,idMarcotaIngreso);
                if(posicion>=0)
                {
...
int mostrarTrabajo(eTrabajo vec[], int tam,eServicio ser[], int tamSer,eBici mas[],int tamMas)
{
    ...
    if (vec != NULL && tam > 0  && ser!=NULL && tamSer>0 && mas!=NULL && tamMas)
    {
        ...
        printf("Id trabajo       marca              Servicio        Precio        Fecha\n");
        for (int i = 0; i < tam; i++)
        {
            if(!(vec[i].isEmpty))
            {
                posicionBici=buscarBiciId(mas,tamMas,vec[i].idBicicleta);
                strcpy(marcaBici,mas[posicionBici].marca);
